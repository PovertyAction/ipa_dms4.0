...
{p 4 6 2}* {opt id()} is always required. {opt sheetname("sheetname")} is required if using file is a {opt xls}, {opt xlsx} or xlsm file.{p_end} 
...
{phang}
{opt sheetname("sheetname")} imports the worksheet "sheetname" from the using file. This is required if the using file is an Excel {opt xls}, {opt xlsx} or {opt xlsm} file. option {opt sheetname()} is ignored if using file is a {opt .csv} or {opt .dta} file.
...
{pstd}
{cmd:ipacheckspecifyrecode} changes the contents of existing variables by recoding other specify values in the parent variable based on instructions specified in the using file. The using file should contain one row per pattern for recoding. Replacements are described by a "parent" column/variable that contains the name of the parent variable to recode, a "child" column/variable that contains the name of the child variable, a "match_type" column/variable that contains the directive for matching, a "match_text" variable/column that contains values to match, a "recode_from" column/variable that contains the original parent value to recode, "recode_to" column/variable that contains the new parent value to recode to and an optional "new_label" column/variable that contains a new label definition if neccesary.
...
{pstd}
For each observation of the using file, {cmd:ipacheckspecifyrecode} checks the value of child value using the values specified in "match_type" & "match_text" and recodes the corresponding parent value from the value in "recode_from" to the value in "recode_to". User can also specify a {helpb varlist} as "parent" & "child" values. However, the number of "parent" or "child" specification per row must match after expansion. 

{pstd}
It is recommended to use the specifyrecode.xlsxm template file from IPA's Data Management System. See {help ipacheck} for information on how to download this file. {cmd:ipacheckspecifyrecode} also accepts Excel xlsx, Excel xls, csv & dta files.
