...
{cmd:ipacheck new} initializes a project's high frequency checks with options to
 create the folder structure, include subfolders for multiple projects, or download 
 inputs files.
 
{title:Options for {it:ipacheck new}}

{phang}
{cmd:surveys(string)} lists all survey forms on which you will run high frequency 
checks. This is typical used for managing projects with multiple surveys. The 
{cmd:surveys()} can be specified as {cmd:surveys(household adult)} to indidate 2 
...
{pstd}The GitHub repository for {cmd:ipacheck} is 
{browse "https://github.com/PovertyAction/high-frequency-checks":here}. The 
{cmd:ipacheck} package contains the following commands:
...
{synopt:{help ipacheckmissing}}export missingness & distintness statistics for each variable{p_end}
...
{syntab:Mata library}
{synopt:{help addlines}}add a lower boder line to a row in an excel file{p_end}
...
{pstd}The {cmd:ipacheck} and all of it's associated content and materials is developed 
